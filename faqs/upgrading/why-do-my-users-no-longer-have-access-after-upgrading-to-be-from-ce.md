...
For security reasons, when the upgrade to BE is performed, any existing users with the **Standard User** role (ie, any users that are not administrators) are assigned to the **Read-Only User** role. This can of course [be changed per-environment](../../admin/environments/environments.md#manage-access) after the upgrade.
