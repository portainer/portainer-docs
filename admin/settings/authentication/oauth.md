...
| Scopes            | A space-separated list of the scopes the OAuth provider requires to return information about authenticated users - for example, `openid profile email`. The exact scopes vary by provider; see your provider's own documentation for more information. |
