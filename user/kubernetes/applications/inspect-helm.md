...
To make changes to, upgrade, or downgrade a Helm deployment, click on the **Edit/Upgrade** button. In the resulting popup, choose the chart version to switch to (your current version will be labeled) and make any adjustments to the values. The default values for the version you have chosen is displayed on the right as a read-only reference. You can preview the manifest that will be created from the updated helm by clicking on the **Manifest preview** header. You can also choose whether to **Rollback on failure** by checking the box.
